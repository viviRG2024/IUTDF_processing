...
if __name__ == '__main__':
    in_dir = r'data\debug\input'
    out_dir=r'data\debug\output'
    ...
    for city_folder in os.listdir(in_dir):
        ...
        if os.path.isdir(city_folder_path):
            sensor_file_path = os.path.join(city_folder_path, f'{city_folder}.csv')
            ...
            if os.path.exists(sensor_file_path):
                ...
                if 'day' in df.columns:
                    ...
                    for date in unique_days:
                        ...
                        city_data.append({'city': city_folder, 'lat':city_lat,'lon':city_lon,'date': date})
                    city_data_df = pd.DataFrame(city_data)
                    city_data_df.to_csv(os.path.join(in_dir, city_folder, f'rainfall_data.csv'), index=False)
                else:
